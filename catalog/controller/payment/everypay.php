...
class ControllerPaymentEverypay extends Controller {
    ...
    public function index() {
        ...
        $fields = array('account_id' => $this->config->get('everypay_account_id'),
                        'amount' =>$order['total'],
        ...
        if ( VERSION >= '2.2.0.0' ) {
            ...
            return $this->load->view($this->config->get('config_template'). 
                                                        '/template/payment/everypay.tpl', $data);
            } else {
    ...
    public function process() {
        ...
        switch($status) {
            case 1 : $this->model_checkout_order->addOrderHistory($this->request->post['order_reference'], $this->config->get('everypay_order_status_id'), '', true);
            $this->response->redirect($this->url->link('checkout/success'));
            break;
            case 2 : $this->model_checkout_order->addOrderHistory($this->request->post['order_reference'], 7, '', true);
            $this->response->redirect($this->url->link('checkout/checkout'));
            break;
            case 3 : $this->model_checkout_order->addOrderHistory($this->request->post['order_reference'], 10, '', true);
